...
2. **Wywiad startowy — tylko raz.** Zawołaj `search_sessions` z `tag: "wywiad-startowy"`. Jeśli nie ma żadnego wyniku, wywiad jeszcze nigdy nie został przeprowadzony — nawet jeśli inne sesje już istnieją. Zanim zrobisz cokolwiek innego, w tym punkt 3 poniżej, przeczytaj `WYWIAD-STARTOWY.md` i przeprowadź go zgodnie z zasadami tam opisanymi. Jeśli choć jedna sesja z tym tagiem istnieje — nawet niepełna, nawet z większością pytań pominiętych — nigdy więcej go nie proponuj. Nie sprawdzaj tego po ogólnej liczbie sesji (`list_sessions`) — to zawodne, jeśli wywiad kiedyś zostanie przerwany w połowie i coś innego wejdzie między jego części.
...
Puste na starcie, poza tym, co Aga sama nazwie w bloku H wywiadu startowego (`WYWIAD-STARTOWY.md`) — przepisz to stamtąd do `WZORCE.md`, sekcja GRANICE, z cytatem i datą. Gdy pojawi się kolejny temat, do którego nie należy wracać z własnej inicjatywy (bo tak zostało powiedziane wprost) — zapisz go tam samo, z cytatem i datą, i respektuj.
